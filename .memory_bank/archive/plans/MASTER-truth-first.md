...
archive_reason: absorbed — детерминизм солвера и единая ТВ-геометрия (T6 truth-first) внедрены
...
- 2026-08-08 — «деплой подряд»: T0–T6 выполнены и закоммичены (fa717bb, 5ee9cf8, dc3b73b,
  ...
  - T4 ✅ (бенч): sku_bench 120 семейств/407 SKU — ИЗМЕРЕНО: CLIP same-series не
    различает (AUC 0.547); челленджер DINOv2 ViT-S/14 zero-shot ТОЖЕ (AUC 0.436,
    retrieval лучше: R@1 67.8/R@5 100) → путь = contrastive-голова (как и предсказал
    рефери §10), ворот нет; sku_identity advisory (кросс-вью C1↔C2 работает).
    ЖДЁТ: contrastive-голова, настоящие positives;
...
- 2026-08-08 (остатки): DINOv2-челленджер прогнан (см. T4); freshness доведён до сборки
  (stale-фиды исключаются из НОВЫХ сетов — mids в feed_guard); ProcTHOR-дуга честно
  переименована в «inspired by» с проверенным provenance (repo@53d5bd4, углов в файле нет);
  прод: trace:prune на remlab-cleanup.timer (TODO закрыт, бэкап cleanup.sh сделан).
