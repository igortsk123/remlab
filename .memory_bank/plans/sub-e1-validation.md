...
4. Fake-doors расширения (решение владельца 2026-07-11): «Ванная» в выборе комнаты, «Посчитать материалы», «Найти бригаду под этот проект» + 10 тест-генераций ванной (блок G).
...
**G. Fake-doors расширения (владелец 2026-07-11: ванная/материалы/бригада; данные — `domain/wordstat-semantics.md` кластеры I/J/K)**
- [ ] `/start` (`app/start/`): карточка «Ванная» рядом с «детская/кухня — скоро» → клик пишет `track("bathroom_interest", ...)` + экран «скоро, оставьте email» (по образцу /soon).
- [ ] Preview: кнопка «Посчитать материалы» → «скоро, оставьте email» + `track("materials_calc_interest", ...)`.
- [ ] Preview (после блока заявки): кнопка «Найти бригаду под этот проект» → форма (город + телефон/email) → запись в jsonb `data.brigade` + `track("brigade_lead", ...)`; первые лиды владелец продаёт 2–3 местным бригадам руками — валидация лид-модели (чек ванной 150–350 тыс.₽, комнаты ~100–250 тыс.₽ — лид дороже товарной комиссии).
- [ ] Тех-тест ванной: 10 генераций фото ванных через текущий пайплайн (промпт restyle без изменений) → разбор `/trace` → ⏸ показать владельцу 5 лучших/худших. Швы плитки и сантехника выглядят правдоподобно ≥7/10 → «ванная» готова к продукту (решение владельца).
- [ ] +3 EventName в `lib/analytics.ts`: `bathroom_interest`, `materials_calc_interest`, `brigade_lead`.
...
- ⏸ ТЕСТ ВАННОЙ: 5 лучших/худших генераций ванных. Вопрос: включаем ванную в продукт и готовим кампанию №2 (`plans/ads-bath-calc.md`)?
- ⏸ ЛИДЫ БРИГАДАМ: первые 3–5 заявок «найти бригаду». Вопрос: кому продаём (2–3 местные бригады), почём?
