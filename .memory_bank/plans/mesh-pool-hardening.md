...
10. **НОВОЕ — тест держится за живой файл запрета:** `tests_pool.case_halt_blocks_start` читает
   боевой `~/scout-scenes/mesh-group-halt.json` и падает, когда в проде настоящая остановка.
   Изолировать (временный HOME или параметризовать путь).
