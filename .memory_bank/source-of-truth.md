...
## Ситуационный канон и приоры (ADR-0112, 19.08) — жёсткая норма
Схема расстановки описывается как **функция × якорь-возможность × форма**; якорь и форма
обязательны в паспорте (`services/planner-solver/rules/templates.json`), функция задаётся зоной.
Приоры практики (`tools/scout/rules/practice_priors.json`) НИКОГДА не суммируются по возможностям
и не превращаются в веса: только локальный тайбрейк при равном верхнем префиксе ключа плана,
включение — после слепых пар и отдельным ADR. При конфликте с любым другим документом
действует эта норма.
