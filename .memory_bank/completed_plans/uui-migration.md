...
## Задачи
- [x] U0 фундамент → деплой (2480bda, CI+prod зелёные)
- [x] U1 примитивы → деплой (e2f6f5c; CI-гейт упал на lint копий UUI — фикс в U2)
- [x] U2 каркас → деплой (a61bbc9, CI+prod зелёные)
- [x] U3 страницы → деплой (b30c3dd, CI+prod зелёные)
- [x] U4 чистка + память + скрин-проверка моб/веб → деплой
...
- 2026-07-31 — «деплой» владельца (+ «делай подряд без спроса»); реклама, вопреки черновику, НЕ шла — обоснование гибрида поправлено (преемственность бренда)
- 2026-07-31 — U0–U3 выкачены четырьмя пушами (авто-деплой CI); отклонение от плана: компоненты
  скопированы в components/base|application (родная структура UUI, alias @/* совпал) вместо components/ui — импорты без правок
- 2026-07-31 — U1 CI-гейт: lint копий UUI (no-empty-object-type) — исправлено type-алиасами в U2
- 2026-07-31 — вкладки /lab оставлены URL-ссылками (серверные, ?tab=) со стилями UUI button-border — RAC Tabs не нужен
- 2026-07-31 — U4: легаси-CSS удалён (337→183 строки, всё на токенах), ui-rules.md переписан,
  CalcForm.tsx удалён (мёртвый, 0 импортов), verify-субагент: «чисто» по 8 пунктам
- 2026-07-31 — скрин-проверка моб (390px) и веб (1280px), 12+2 скрина: поймана и исправлена
  регрессия (бейджи «скоро» в шапке не прятались на мобиле → 3 ряда навигации)

## Completion summary
Полный переход UI на Untitled UI React за одну сессию, 4 атомарных прод-деплоя (U0 фундамент →
U1 примитивы → U2 каркас → U3 страницы) + U4 чистка. Палитра-гибрид зафиксирована в brand.css
(терракота 500/600 = прежние CTA пиксельно). Легаси-CSS демонтирован (337→183 строки, только
хелперы на токенах); все интерактивные элементы — компоненты UUI (react-aria: фокус, Esc,
a11y из коробки). Метрика-цели 1:1 (verify подтвердил). Упрощено: шалфей упразднён (один
бренд-акцент); RAC Tabs не понадобился (вкладки — ссылки). Отложено: PRO ($349) — решение
владельца; шапка .site-* осталась bespoke-CSS на токенах (механика коллапса не тронута).

### Уроки (ОБЯЗАТЕЛЬНО)
- Lint копий чужого кода гонять полностью ДО пуша (U1 упал в CI на no-empty-object-type; смотрел tail).
- Next standalone локально без .next/static → CSS 404 (9 байт «Not Found») — выглядит как катастрофа; проверять через `next start`.
- Препролёт Tailwind: маркеры списков/рамки нативных полей гаснут; legacy вне @layer сильнее слоёв — мост безопасен.
- Перенос CSS в утилити-классы теряет media-query поведение молча — скрин-проверка обязательна (поймала шапку).
- Playwright на DEV-VM: только кеш chromium-1234 через executablePath (ubuntu26.04 не поддержан).
