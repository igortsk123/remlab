...
- **Сеты**: v1 заморожен; v2 «как дизайнер» (ADR-0044) + валидатор состава и пригодности
  (ADR-0046, `../domain/living-room-checklist.md`); зелень/картины — НЕ товары.
...
- **3D товаров** (ADR-0060): кому плоское фото врёт — GLB через `fal-ai/trellis` (≈$0.02, кэш на
  SKU), ракурсы рендерим у себя без видеокарты (`tools/scout/mesh_render.py`, 1,3 с на вид).
