...
Дизайн · Стили · Советы (1100px); десктоп центрируется, зум+лаборатория справа; ≤700px скролл
сворачивает ряд (урок 7 [[lessons]]); ≤480px компакт. Активный раздел — заливка, не кольцо.
Иконки владельца `public/icons/` + фавиконка — [[access-and-integrations]]. Главная = «о проекте».
...
- **`/sovety`** — плитки-советы ПЛЕЙСХОЛДЕРЫ (тексты позже, М7). **`/lab`** — «Моя лаборатория»:
  сметы сессии + «Мои дизайны»; `/estimates` → redirect `/lab` (ADR-0036); аккаунт позже.
