...
`draft_render.FIXTURE_MESH`. 
**ADR-0168/0173:** все модели с ДИСКА образа, нода в интернет не ходит.
**Приёмник и стопоры — ADR-0175:** приёмник проверяется ДО раздачи (`sink_health`, канарейка),
красный приёмник = инфра, не вина нод; тариф/окно/цена — `tools/scout/rules/salad-groups.json`;
стоп-файл старше автостарта, окно старше «нет тёплых нод». **ADR-0176:** обрыв SSH с
диагнозом; конвейер singleton, перезапуск через `mesh-draining`; цена по ОПЛАЧЕННЫМ нодо-часам
(`tier_compare`); образ `localpaint2`. «Running» ≠ работает — судить по `gpu_seconds`/`done`.
...
`mesh-dino-baked` · ADR-0129…0134, 0146, 0168–0170, 0173–0176.
