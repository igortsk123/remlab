...
**22.08 (лечение банка):** `heal`/`refresh` копируют ГАБАРИТЫ нового товара + конверт банда для
якорных ролей; контракты комплекта («кресло 2» = экземпляр основного, снятый pod оставляет
`coverage_gap`) — детали в Tier 2 (`tools/scout/sets_incremental.py`).
