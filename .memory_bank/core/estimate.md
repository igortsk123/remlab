...
## Что это
Расчёт материалов/стоимости → смета с реф-ссылками. Мастер — `../plans/MASTER-cost-first.md`; М1 построен.

## Два входа
- **А — калькуляторы** (`/calc/[kind]`): обои/плитка/краска/ламинат; сопутка `companions.ts`;
  стены добавляются из карточки размеров. **Без параметров материала количество НЕ считаем**
  (`qtyUnknown` + «? ед → что сделать»; в смету идёт площадь) — ADR-0034.
...
`lib/calc/*`, localStorage). UX: проёмы скрыты; плитка — размер в СМ (хранение мм), цена за
м²/шт/упак; ламинат — цена за м² (ADR-0030). Хвосты: итог → сопутка → «найдём дешевле» → виз.
Детали — ADR-0019/0027/0028/0030; роадмап `calc-materials-roadmap.md`.
