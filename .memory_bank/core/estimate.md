...
`parse-product.ts`: regex+OG → JSON-LD → ИИ дочитывает пустые поля. UI долго → «подождите
несколько секунд»; неудача → ручной ввод (загрузки файла нет). ⚠️ Ozon/WB блокируют IP пула
(капча) — нужен прокси-анблокер (Bright Data/Zyte); Леруа/Петрович читаются.
