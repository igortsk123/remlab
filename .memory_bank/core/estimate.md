...
- **Late-binding реф** `/go/[eid]/[iid]`: лог `link_clicks` + 302 на реф из `link_routes`
  (пусто → прямая); мультисеть (Гдеслон/прямые/Admitad/ePN).
...
плитка — инлайн-результаты стен/пол, размер в СМ (хранение мм), цена за м²/шт/упак; ламинат —
цена за м² (конвертация упаковка→м², стоимость целыми упаковками; ADR-0030); «?»-подсказки.
Копирайт по kind (`CALC_META`); хвосты: итог → сопутка → «найдём дешевле» → виз. Детали —
ADR-0019/0027/0028/0030; роадмап `calc-materials-roadmap.md`.

## Чтение ссылок (ADR-0031)
direct → прокси `PARSE_PROXY_URLS` (cap 2 МБ) → Ozon/WB (JS-антибот) → `needs_file` (юзер грузит
сохранённую страницу → `/api/calc/parse-html`); парс `lib/calc/parse-product.ts`: regex+OG →
JSON-LD → ИИ дочитывает пустые поля (`link-content.ts`).
