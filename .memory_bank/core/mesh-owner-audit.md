...
постер → вертящаяся 3D-модель по клику, одна кнопка «переделать». Вход — кука
`/lab/mesh-review` (`lib/mesh-review/auth.ts`). Решения владельца 05.09: 3D, не картинки;
переделка — в ОБЩУЮ очередь; **2 ручные переделки на товар за всё время** (авто-перегон не
считается); прогресс «просмотрено» считать; исходники на DEV не удалять.
...
`components/lab/MeshAuditCard.tsx` (GLB — только по клику), model-viewer вендорен в
...
**Tier 2:** план `../plans/mesh-owner-audit.md`; аудиты Codex — `../_intake/codex-prompts/q-mesh-owner-audit*.md`.
