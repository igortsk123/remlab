...
## Структура (факт)
- `/app`: `page.tsx` (главная). **Смета-first (M1):** `/calc` хаб, `/calc/[kind]`, `/calc/remont` (вход Б), `/e/[id]` чек-лист, `/estimates` (redirect → `/lab`, ADR-0036), `/go/[eid]/[iid]` редирект. **Навигация (ADR-0017):** шапка `components/SiteHeader.tsx` (в `layout.tsx`); каркасы `/styles` (`components/StyleQuiz.tsx`+`lib/styles/quiz.ts`+`app/styles-actions.ts`), `/sovety`, `/lab`. **Legacy AI-флоу (М5):** `/start`, `/p/[id]/{brief,select,style,preview,paywall}`, `/rooms`. `/api` (health, p, trace).
