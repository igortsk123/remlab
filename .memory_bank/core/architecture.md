...
## Структура (факт)
- `/app`: `page.tsx` (главная «о проекте целиком»). **Смета-first (M1):** `/calc` хаб, `/calc/[kind]`, `/calc/remont` (вход Б), `/e/[id]` чек-лист, `/estimates`, `/go/[eid]/[iid]` редирект. **Навигация (ADR-0017):** сквозная шапка `components/SiteHeader.tsx` (в `layout.tsx`); каркасы `/styles` (игра `components/StyleQuiz.tsx` + `lib/styles/quiz.ts` + `app/styles-actions.ts`), `/sovety`, `/lab`. **Legacy AI-флоу (М5):** `/start`, `/p/[id]/{brief,select,style,preview,paywall}`, `/rooms`. `/api` (health, p, trace).
- `/modules` — store (projects) + **estimate** (`repository.ts`: memory/pg). visual-generation/generation-job/ideas/room-analysis — AI-флоу (М5).
- **Смета (v0.4):** `contracts/estimate.ts`; `lib/estimate/{calc,links,companions}`; `lib/pricing/works` (плейсхолдер); `app/estimate-actions.ts`. Наружу ТОЛЬКО через `/go/` (late-binding реф).
- `/db` (schema + init 001-004), `/contracts`, `/lib`, `/e2e`, `/tools`, `/docs`.
...
## Цели (НЕ реализовано)
Inngest · матчинг/каталог/платежи/export · Vertex/fal · YooKassa (paywall-заглушка) · Sentry · `/eval` · точный Cost Engine (вход Б = плейсхолдер `lib/pricing/works`, план `pricing-db-ru`).
