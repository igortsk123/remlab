...
извлечённых JSON-пакетов книг: атомарные claims с провенансом (страница/фигура, сила claim'а,
авторитет IRC/NKBA, конфликты) → retrieval-слои. **Production-правила солвера НЕ меняет** —
...
писался. Корпус и спека на диске, untracked в git — закоммитить в KB0.
...
- `remlab_knowledge_db_v1/spec/SPEC_source_kb_v1.md` — контракт пайплайна (фазы 0–11,
  инварианты A–G, ID-policy, commit-протокол). ⚠️ Обрезан в фазе 9 — хвост запрошен у владельца.
...
**Посадка кода (план):** новая подсистема services/knowledge-db (Python 3.14, pydantic +
jsonschema + rfc8785, pytest, venv ~/venvs/kdb); артефакты — runs/<run_id> в KB-каталоге.
