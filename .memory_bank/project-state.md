...
- **Мебельный трек v2 ОДОБРЕН (2026-08-01, ADR-0042):** Гдеслон дал доступ к 14 магазинам;
  3 draft-плана: [[gdeslon-catalog]] (фиды→products+клики) → [[ergonomics-planner]] (Gemini+
  Holodeck-солвер) → [[living-room-sets]] (~20 сетов, Excel-курация). Код НЕ начат.
  **Ждут владельца (блокеры Ф0/Ф2):** выгрузки в кабинете `/export_files/` (14 шт), токен
  `_gs_at`, hash/erid диплинка. Эндпоинты/mid — `core/access-and-integrations.md`.
...
- **UX-полировка по фидбеку (2026-07-30…31, ADR-0030/0034–0039):** одна «сохранялка», `/lab` —
  вкладки с тизерами + «Мой стиль», единые имена (полка = Лаборатория, документ = «Расчёт»),
  без молчаливых дефолтов. Детали — `core/estimate.md`, `core/user-flow.md`, `decisions.md`.
