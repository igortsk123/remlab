...
- **Запуск (launch-prep, 2026-07-28): П1–П5 ВЫКАЧЕНЫ** — П1 витрина-заглушки; П2 «Моя лаборатория»
  ...
  авто/вручную autoKeys, «Нашли: <товар>»). Осталось: П7 лид-канал (нужны токены ботов), П6 финал.
- **Калькулятор материалов v2 (К0–К6) сильно доработан по UX (2026-07-21…22, ADR-0018–0027):** дробный
  ввод (`NumInput`), липкая шапка итогов, плитка стены+пол разными плитками, блок ссылки с
  АВТОЗАПОЛНЕНИЕМ параметров по ссылке (regex-парсер + **ИИ-фолбэк OpenAI** ADR-0026), выделенная
  лид-карточка «найдём этот товар дешевле» (e-mail-модалка + боты). **ADR-0027:** проёмы убраны из UI
  у всех видов (запас на подрезку); плитка — инлайн-результаты стен/пол, «? шт» без размера. `core/estimate.md`.
- **Прод:** https://remont-lab.online — версия = git SHA (АВТО-деплой на каждый push в main,
  нативный arm64-раннер, с 2026-07-28); раньше вручную
...
- **Калькулятор материалов v2 (2026-07-12, ADR-0018; серия К0–К6) — ВСЕ 6 фаз в проде** (v193856):
  мультикомната + геометрия с проёмами/зонами + полные параметры + формулы (golden) → смета (v2 —
  основной на `/calc/[kind]`, localStorage); автозаполнение по ссылке (К4); визуализация 60₽ (К5,
  скелет) и «найти дешевле»+лиды (К6, скелет). Скелеты требуют ключей владельца: YooKassa (оплата),
  токены ботов Telegram/MAX; ПДн-часть — TODO/юрист. `core/estimate.md`, `completed_plans/calc-K0..K6-*`.
