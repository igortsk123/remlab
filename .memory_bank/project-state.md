...
практики — локально и в тени. Библиотека `/test/canons/` (64 нумерованные карточки) рисуется
БОЕВЫМ кодом планов и проходит `validate()`; паспорт — 12 зон / 59 схем, покрытие полное,
спящих 4 (ждут данных каталога и вертикальной инженерии). Сводка — `core/layout.md`.
