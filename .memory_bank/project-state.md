...
**Реклама (2026-07-11):** получены доступы Яндекс Wordstat/Директ/Метрика (общий аккаунт с
v0-health-card, значения в `_secrets/ACCESS.md`) и собрана семантика ниши — прямой AI-спрос
~15–18k показов/мес, «дизайн проект квартиры» 48k, бренды конкурентов ~60k, сезон ремонта
июль–ноябрь (идёт). Сводка — `core/marketing-acquisition.md`, данные —
`domain/wordstat-semantics.md`. Счётчика Метрики для remont-lab.online НЕТ (пререквизит).
Следующий шаг — draft-план настройки Директа.
