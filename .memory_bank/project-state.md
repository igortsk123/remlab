...
## Расстановка мебели — текущий фокус (2026-08-17)

**Свод №13 (`plans/MASTER-zones-v7.md`, ADR-0107): Q0–Q5 задеплоены (4c511ea)** — метрики «как видит
владелец», identity-адаптер банка, медиапригодные кресла + сертификаты семейств, `plan_key_v2` shadow,
второй pod как атомарный комплект (пара 3/4 + столик 2) с контрактом смысла; контракт позы в экспорте
(планы → 3D → LLM). Гейт: 269+3 честных, TIMEOUT 0 @6 воркеров (RAM VM 4 ГБ), dining 239, медиа 269,
second_pod 57 full_valid / 17 с причиной, 0 голых pod. **Далее:** Q6a–e (каталожная волна: банкетка/
кушетка/консоль/раскладной), Q7 слепой раунд 2 (по 20 пар; v2 → default по итогу).
**Ждёт владельца:** карантин nonton.ru (фид сломан с 11.08, 1076 позиций во всех сетах); платное
обогащение дельты (сначала `--sample 0`). Работа с Codex — по правилу «советник до действий»
(`core/access-and-integrations.md` § Codex; постоянные сессии layout/catalog).
Галерея `/test/acceptance-plans/`, экспорт `/test/plans-export.zip`.
