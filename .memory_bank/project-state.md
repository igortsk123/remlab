...
- **Арбитраж рефери завершён и принят целиком (08.08, ADR-0076/0077, `plans/referee-hardening.md`)**:
  финальная оценка 9.2/10, «архитектуру не менять, вопрос почти закрыт». Три партии внедрены:
  демоции FLOOR_OVERFILL/SOFA_SLIVER, чеки камин/окно (S1/S2), ярусы = приоритет удержания
  (дроп dining/storage, обеденная атомарна ≥2 стульев), мёртвая зона локализована, wall-back из
  данных, ТВ distance-first, пропорции hard/soft, 47 кодов в реестре. Найдена и убрана
  контаминация A/B (DFS-фолбэк под лейблом zoned — ADR-0076 + test_engine_purity).
  Правило владельца: расходиться с рефери — только с самостоятельным пруфом из источников.
  Приёмка рефери-пакета: **243/252 чистых (96.4%)**, все провалы солверные. Очередь плана:
  вытянутые комнаты (солвер), слой регулярности, human blind A/B (решение владельца).
- **Исследование рефери «новые роли» → `plans/inventory-additions.md` (ADR-0078, in_progress)**:
  ...
  падает со 107 сцен к ~30%-уровню; ИДЁТ перегон приёмки с фиксами; дальше — страница десятки,
  xlsx --date=2026-08-08, коммит; волна A8 (пересборка сетов, картины подтянет крон каталога).
...
- **`plans/MASTER-truth-first.md` (ADR-0079/0080, in_progress) — волны T0–T6 выполнены 08.08**
  («деплой подряд», 7 коммитов fa717bb…adf5c87): предохранители конвейера (батч «completed
  0 готовых» ловится — причиной был исчерпанный биллинг, гейт разклинен, 1 859 карточек ждут
  пополнения); evidence-резолвер размеров (0 битых, +4 610 размеров, divan.ru ожил); gold-human
  инструменты (разметка — решение владельца, /test/gold-human/annotate.html); мост замер→солвер
  + real-бенч + фаззер (нашёл и починен _ROOM_BAND-недетерминизм); SKU-бенч (ИЗМЕРЕНО: CLIP
  same-series не различает, AUC 0.547 → ворот нет до челленджера); set-level оптимизация
  (15/126 улучшены) + шторы (дыра 126/126 закрыта); единая ТВ-геометрия planner/tv.py,
  constraint-CI, 87 тестов. Открыто (данные/владелец): разметчики gold, 100–300 планировок,
  SKU-эмбеддер, целевые суммы корзины.
