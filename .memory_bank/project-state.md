...
- **Калькулятор материалов v2 (2026-07-12, ADR-0018; серия К0–К6)** — детальный расчёт: мультикомната,
  геометрия с проёмами/зонами, полные параметры материалов, формулы с golden-тестами, результат → смета.
  **К0–К3 в проде** (v2 — основной на `/calc/[kind]`, localStorage); К4 ссылки, К5 виз.60₽, К6 «дешевле» —
  далее. `core/estimate.md`, `completed_plans/calc-K0..K3-*`.
