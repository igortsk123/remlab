...
- **Z5 ✅, решение принято (ADR-0075)**: приёмка 252 фикс-сцены (оба движка, одни Z4-составы) —
  ...
  (severity-реестр+тест, фикс-эргономика, size-bands→приоры, SHA-provenance, pair_symmetry).
  Идёт прогон zoned+W; очередь: вердикт-фиксы владельца (камин ≤450 enforced, диван-у-окна
  enforced, выравнивание стульев) → финальный прогон → страница десятки с парами.
- **Разбор внешних систем** + полный аудит-ответ: страница
  https://remont-lab.online/test/zones-vs-external/ (`tools/scout/pages/`); пакет для рефери —
  `rules_export.py --referee` (ответ на аудит, 7 спорных вопросов, контекст приёмки).
