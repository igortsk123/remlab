...
- **А0 выполнена (06.08):** незабранный оплаченный батч (2 000 товаров, весь свет) записан в
  БД — обогащение покрывает **25 028/25 028** active-пула, качество 0.86; батч-дисциплина
  ADR-0073 (гейт отправки + журнал забора). **Дальше: А1** (цепочка свежести: крон забирает
  батчи, дельта запускает переобогащение, алертинг) → А2 (кашпо/ковры) → А3 (один движок beam
  + зрячая метрика) → А4 (виз: пост-QA финала, ретраи) → А5–А7.
- Ключевые находки аудита живут в: `core/catalog.md`, `core/layout.md`, `core/furniture.md`,
  `domain/catalog-enrichment.md` (все обновлены 06.08), уроки 190–192 в `core/lessons.md`.
