...
- **Блокер обогащения (найдено 07.08):** третий vision-батч 1 859 карточек (divan.ru/mdm-complect)
  завершился `completed` с 0 готовых / 1859 ошибок → `enrich.fetch` падает 404, гейт батчей закрыт
  (`enrich-batch-id.txt` не пуст) — завтрашний cron не отправит новый пакет; `enrich_wait.sh` это
  сбоем не считает, алерта нет (урок в `core/lessons.md`). Разбор и сводный документ конвейера для
  рефери — `plans/referee-hardening.md` (раздел «Материалы для рефери», ссылка на artifact).
