...
## Где
- **Стадия:** концепция **v0.4 «Смета-first»** (ADR-0016). **Ядро сметы М1 ПОСТРОЕНО**
  (калькуляторы вход А, вилка вход Б, смета-чек-лист, /go/ редирект, лендинг переделан) —
  `plans/m1-estimate-core.md`. Дальше: pricing-db (нормативы работ, в очереди), М5 (виз./мебель
  — legacy AI-флоу `/p/*` жив). Прежние пивоты: v0.3 ADR-0014.
