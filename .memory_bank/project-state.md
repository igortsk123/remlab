...
- **ОЧЕРЕДЬ РАБОТ (выстроена владельцем 2026-08-07, выполнять подряд):**
  1) R1 полные функциональные зоны + 2) R3 состав сетов по зонам ([[layout-rules-v2]] —
  основной прирост логичности); 3) Э8 полигональные комнаты ([[layout-polygon-rooms]] —
  выход на реальные квартиры, 3 референса-фикстуры); 4) хвост: backtracking 26–30
  ([[layout-engine-gaps]]), веса priors в score, калибровка DUMB_T вердиктами.
- **Прочие остатки:** backtracking 26–30 ([[layout-engine-gaps]], стенка/комод ×8);
