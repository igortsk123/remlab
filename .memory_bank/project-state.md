...
- **Арбитраж рефери завершён и принят целиком (08.08, ADR-0076/0077, `plans/referee-hardening.md`)**:
  ...
  ИДЁТ финальный прогон 252 с полным пакетом (~97% чистых на середине); после — страница десятки,
  финальный xlsx (`rules_export.py --referee --date=...`), коммит. Очередь плана: вытянутые
  комнаты (солвер), слой регулярности, human blind A/B (решение владельца).
