...
- **Мебельный трек v2 — разведка ГОТОВА, конвейер согласован (2026-08-01, ADR-0042/0043):**
  каталог 7 магазинов (87,6 тыс. тов., размеры 87–100% мебель) в dev-БД; 21 сет автосборкой по
  справке владельца; конвейер витрины A→B→C→D в работе — [[viz-pipeline]] (in_progress, тюнинг).
  Прод-код НЕ начат ([[gdeslon-catalog]]/[[ergonomics-planner]]/[[living-room-sets]] draft).
  **Ждут владельца:** утверждение мудбордов (21 сет); подключение divan/askona/ormatek; заявка
  cozyhome (ковры/шторы); пересоздание МЁРТВОГО Gemini-ключа. Сводка — `core/furniture.md`.
