...
- 29.08 В РАБОТЕ: возврат in_stock жертвам битого маркера health (119 divan.ru) идёт фоном по перепроверке страниц (~2с/товар, restock.log); после него пере-прогнать `sets_incremental --enforce-contracts` вхолостую — ожидание: «вне контракта» падает с 119 до единиц. Импорт health.py исполняет полный скан — результат возврата проверять по restock.log, не импортом.
