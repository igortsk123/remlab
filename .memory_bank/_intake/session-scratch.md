...
- 2026-08-10 (деплой MASTER-source-kb, в процессе): KB0-KB4 выполнены и запушены
  (каркас services/knowledge-db, preflight, merge, 3347+2 атомов со стабильными ID,
  обогащение 2B-2E, реестры словаря/авторитетов/origins/оверлеев; 45 pytest-гейтов).
  Регрессии спеки R030/R096/R068/egress/Hall/R014-R076 — зелёные.
- LLM-фактура: gpt-5.6-luna, условия+словарь+авторитеты ~$0.12; судья пар KB5a идёт
  в фоне (~60%, прогноз волны ~$4-5). Гонка снапшотов реестра вердиктов найдена и
  починена (single-writer; сырой кэш ответов спасает деньги при рестарте).
- KB5b/KB6 код написан (семьи по anchor-подписям из замороженных реестров — ID-стабильность;
  оракул полноты 7B), ждут выхода KB5a. Не закоммичены до зелёных гейтов.
