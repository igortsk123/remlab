...
## 2026-08-17 — Свод №13 (вытеснено из снимка 21.08)
Q0–Q5 + Q6a задеплоены (`plans/MASTER-zones-v7.md`, ADR-0107/0108). Владелец 17.08: «никаких
допусков — только каноны» (столик всегда по центру, реестр канонов `/test/templates/`), подписи
«только куда повёрнут», pod-комплекты (пара кресел + столик 2) с лечением, роли каталога по листу
категории (кресла divan.ru вернулись). Экзамен того дня: 264/272, 8 TIMEOUT в XL (закрыты к
20.08), pod достижим 62/74, quiet выбран 14. Ускорение цикла ч.1: `run.sh smoke|perf|scenes|render`,
снимок банков, flock, 10 воркеров.

## 2026-08-19…21 — каноны как фокус
Q11 (библиотека канонов, ADR-0109/0111) завершён; Q12 (ситуационный канон, ADR-0112/0113/0114) —
паспорт приведён к якорям и формам, библиотека консолидирована (карточка = группа×форма, зеркала
слиты, `seating.default` расщеплён на 7, запасные исходы — `situational_fallback`), заведены
каноны `reading.window_pair`/`bay_pair`, `decor.window_plant`, `reading.wall_vignette`, разбужены
`storage.corner_tower` и два `window_seat`. Волны пайплайна и цепочка свежести каталога
переведены в `partial` — работа не велась с 06.08.



## Вытеснено из снимка 31.08
...
### 31.08 — простой пула мешей (вытеснено из снимка 02.09)
batch-машин не было ~9,5 ч (первая в 16:00), пришедшие качали 13–19% за 5 ч и были
пересажены `cull_slow_pulls`. Прогон держал список нод, снятый на старте → план
`mesh-dynamic-node-pool` (динамический пул + честный учёт заданий). Продолжение
сюжета — ADR-0157 (02.09): то же правило пересадки выкашивало пул уже на новом образе.

### До 02.09 — вытеснено из снимка (пилот мешей и спящие звенья)
- Пилот: 199 товаров с мешами (01.09), галерея /test/mesh-pilot10/ — 156 карточек (по одной на
  МОДЕЛЬ товара: варианты цвета/размера свёрнуты, 40 повторов); вход HD (ADR-0136).
- Спящие звенья до сигнала владельца: маски (`CUTOUT_BACKEND=salad`, нода занята пилотом мешей
  в соседней сессии), подача батча на Salad и приём в `asset_revisions`.

## Расстановка мебели, этап 21.08 (перенесено из project-state 02.09 — снимок ≤12 КБ)
...
## 02.09 — вытеснено из снимка
Детали отменённого ремонта мешей (ADR-0143): копии распаренных моделей лежат в
`~/scout-scenes/mesh-repairs-parked/`; план цвета включал снятие ножей, патч seed/CFG покраски,
экспозицию входа и опыт с 4→12 заданиями.
