...
## Содержание
1. Формат записи
2. YYYY-MM-DD — <веха одной строкой>
3. 2026-09-04 — Работа над ошибками пула мешей (план `mesh-pool-hardening`, ADR-0175/0176/0177)
4. 2026-09-03 — Пул мешей заработал; аудит загрузки и честности каталога
5. 2026-09-02 — Состояние прогона мешей перед разгоном
6. 2026-07-02 — Пивот бизнес-модели v0.2 → v0.3 (ADR-0014)
7. 2026-07-02 — Трейсинг AI-пайплайна реализован и задеплоен в прод (ADR-0013)
8. 2026-07-01 — Stage 1 продукт M0–M8: реальный ИИ, Postgres, ручной прод-деплой, observability
9. 2026-07-01 — Bootstrap S1–S4 завершён (`completed_plans/remlab-bootstrap.md`)
10. 2026-07-11 — Яндекс-доступы, семантика Вордстат, починка CI gate
11. 2026-07-11 — М1 «Смета-лист» построена (v0.4, ADR-0016)
12. 2026-07-28 — launch-prep: зонт П1–П7 закрыт, платформа готова к запуску
13. 2026-07-30 — UX-полировка по фидбеку владельца + чтение ссылок П5b
14. Где
15. Концепция v0.4 «Смета-first» (ADR-0016, 2026-07-11; мастер: `plans/MASTER-cost-first.md`)
16. 2026-08-09 — вытеснено из project-state при /memory-check
17. 2026-08-16/17 — Свод №13 Q0–Q5 (ADR-0107)
18. 2026-08-14 — вытеснено из project-state 17.08 (предыдущий фокус расстановки)
19. 2026-08-17 — Свод №13 (вытеснено из снимка 21.08)
20. 2026-08-19…21 — каноны как фокус
21. Вытеснено из снимка 31.08
22. Демо-планировщик и рендер (26–28.08)
23. Расстановка мебели, этап 21.08 (перенесено из project-state 02.09 — снимок ≤12 КБ)
24. Расстановка мебели — текущий фокус (2026-08-21)
25. 02.09 — вытеснено из снимка
26. Вытеснено из снимка 03.09 (загрузка каталога переписана, см. ADR-0171)
27. Каталог и стиль (2026-08-06)
...
## 2026-09-04 — Работа над ошибками пула мешей (план `mesh-pool-hardening`, ADR-0175/0176/0177)
Пул за двое суток вышел на 100 мешей/час, но каждую ночь вставал по нашей вине. Починено:
приёмник проверяется ДО раздачи GPU (канарейка PUT 1 МБ), обрыв SSH несёт диагноз и не повторяет
генерацию вслепую, три стопора живут по одному своду правил (`salad-groups.json`), конвейер —
singleton с горячим перезапуском через `mesh-draining`, цена считается по ОПЛАЧЕННЫМ нодо-часам,
шаги разбора не умирают от earlyoom (RSS 10 ГБ → 465 МБ), сервер чистит теги CI и сторожит диск,
приёмник переехал в compose, образ `cu124-localpaint2`, группы пересозданы на него
(`mesh-low-4/5`, `mesh-batch-3`). Владелец поправил два вывода: batch не удалять, а держать по
окну 09–15 UTC (ADR-0177), и `mesh-batch-3` работала из-за `autostart_policy`, а не вопреки окну.
Уроки 396–404. Остаток и суточные замеры — в плане.

## 2026-09-03 — Пул мешей заработал; аудит загрузки и честности каталога
**Меши:** 376 → 1487 за день, 100 мешей/час на 20 нодах (02.09 было 22 за сутки и ни разу больше
одной ноды), GPU 87–92 %, прогрев 132–272 с, 1.35 ₽ за меш по оплаченному времени. Вычищены:
лишняя копия чекпойнта, DINOv2 и веса покраски из интернета, свои же пороги пересадки, зомби при
живом пуле (ADR-0168/0170/0173/0174).
**Загрузка каталога** (план `catalog-load-hardening`, ADR-0171/0172): фид — источник
HD/артикула/описания (HD 12 000 → 18 733 in_stock), API — только комиссия; отпечатки v3 и
`enrichment_status` вместо обнуления версии; порог 70 % по магазину и журнал прогонов; одна
транзакция; оси размеров по магазину (глубина мебели tvoydom 0 → 95 %, без размеров 2 148 →
...
`footprint.py` без дефолтов — банк №3: 126 сетов, 2 479 позиций, 0 напольных без размера.
Сводки — [[catalog]], [[stock-and-dims]].

## 2026-09-02 — Состояние прогона мешей перед разгоном
Очередь полная и по регламенту (ADR-0153): 11 704 задания, ярусы владельца, `mesh-queue-v1.json`,
курсор 30. Готово мешей 376, осталось 7 410 в ролях слотов. Демо flat215 закрыто (9 позиций
владельца сделаны 01.09).
...
- Пилот: 199 товаров с мешами (01.09), галерея /test/mesh-pilot10/ — 156 карточек (по одной на
  МОДЕЛЬ товара: варианты цвета/размера свёрнуты, 40 повторов); вход HD (ADR-0182).
...
## Вытеснено из снимка 03.09 (загрузка каталога переписана, см. ADR-0171)
