...
## 2026-09-05 — апгрейд кита v1.6.0 → v1.7.0
Команда: upgrade.sh

- UPGRADE  kit-owned файлы — обновлено: 2, добавлено: 0, конфликтов: 0 (*.kit-new)
...
- SPLIT    anti-patterns.md (220 КБ) → anti-patterns.md (13 КБ: код-антипаттерны §1–7, M1–M5 + ссылки) + lessons/{mesh-salad-pool, catalog-stock, layout-solver, viz-render, demo-planner, devops-deploy, memory-process, estimate-ads-leads}.md — 449 записей дословно (сверка: 776 строк, sha256 совпал); lessons/README.md — карта «номер → файл», пропуски (215–217, 220–222, 239–242, 346) и дубли (57, 169, 303, 308–312, 353–358, 394–396) признаны явно; следующий номер 416; core/lessons.md — маршрутизатор ≤3 КБ
- MOVE     _intake/codex-prompts/* и корневые codex-*/answer-*/prompt-*/*-referee-* → _intake/codex/; owner-*/dialog-*/blind-*/self-analysis-*/*.json/*.txt → _intake/owner/ (basename сохранены, ссылки в 39 доках переписаны)
- ADD      ADR-0187 (порядок ступеней: М5 раньше М2–М4), ADR-0188…0194 (реестр поколений мешей и приёмка владельцем — из блокнота сессии mesh-owner-audit), ADR-0195 (гард Bash pkill -f); уроки 414–415 (шаг CI + инструмент одним коммитом; общий Tier 0 не ужимать вслепую)
- FIX      финал: блокнот сведён (27 записей → ADR-0187…0195, уроки 414–418, mesh-pool-ops, снимок); план → completed_plans/; `_kit/gate-mode.txt` = block (решение владельца: после ручного /memory-check); сводки сверены с кодом verify-агентом (11 расхождений закрыто, last_verified=05.09 у 18 доков)
- FIX      по дрилл-тесту (5/5): пары Tier 1↔2 (domain/stock-and-dims → core/stock-and-dims, domain/pricing-works-ru → core/estimate), порядок томов в decisions.md, wikilink по basename, старые номера внутри томов, цена меша в снимке
