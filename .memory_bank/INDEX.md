...
## Ключевые исходники
`docs/master-brief-v0.3.md` — МАСТЕР (бизнес) · `docs/tech-spec-ts-stack.md` — инж. спека ·
`docs/cjm-ux-v0.2.md` — CJM/экраны · `docs/market-research-ru-uk.md` — рынок · `docs/DECISIONS.md` — ADR.

## Планы
Мастер-планы: `plans/commercial-master-plan.md` (Деньги: Э0–Э8 + реестр подпланов `sub-*`) и
`plans/MASTER-roadmap.md` (техядро fit, Ф0–Ф8). Исполнение подпланов — `guides/execution-playbook.md`.
Цикл: draft → «деплой» → completed → `completed_plans/` (гейт: `/memory-check` + audit чисто).

## Обслуживание
Конец сессии — `/memory-check`; уборка — `/memory-cleanup`; шапки — `METADATA_SCHEMA.md`;
`core/`=Tier 1, `guides/`=процессы, `archive/`=старое, лог — `changelog/memory-log.md`.
