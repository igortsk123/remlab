...
## Содержание
1. Google Gemini — активен ✅ (Stage 1, M0)
2. OpenAI — ключ есть ✅ (используется автопилотом рекламы, НЕ приложением)
3. Observability — PostHog (ADR-0012)
4. Affiliate-сеть и фиды (М0/v0.4 — КЛЮЧЕВАЯ; доступ есть, код ещё НЕ написан)
5. Трейсинг пайплайна + imagor (ADR-0013)
6. Деплой / CI-доступы (ADR-0011)
7. Яндекс: Wordstat / Директ / Метрика — доступ есть ✅ (проверен 2026-07-11)
8. Цены (ориентир, 2026)
9. Гдеслон: свежесть фидов и наличие (2026-08-02, ADR-0045)
10. UI-иконки (перенос из Tier1-сводки, 2026-08-02)
11. fal.ai — inference-API (добавлено 2026-08-04)
12. Реестр
13. Codex (OpenAI CLI) — постоянная сессия-советник (16.08.2026)
14. Sketchfab — модели-заглушки для сцены (01.09.2026)
15. Vercel AI Gateway (26.08)
16. fal.ai (2026-08-05 → 28.08)
17. Рендер-сервис: фоновые задания, реестр кадров, миниатюры (02.09)
18. Гдеслон — фиды vs API (сверено 03.09, ADR-0171)
...
- **Сессия проекта:** `01a00a62-33e2-7051-93c6-37bff5c6937e` (онбординг 16.08: прочитал CLAUDE.md, INDEX, ADR-0099…0106,
  MASTER-zones-v7, свои аудиты, карту кода; конспект — `_intake/codex/codex-onboarding-notes.md`).
...
- **✅ 02.09.2026 — СОВЕТНИК СНОВА РАБОТАЕТ.** Постоянная сессия ответила по вопросу про пул
  Salad: 14.7 КБ, со ссылками на код репозитория и документацию Salad, за ~9 минут
  (`_intake/codex/q-salad-mesh-pool.answer.md`). Что делали иначе: длинный
  структурированный промпт с замерами и явными вопросами, `-o` в файл, таймаут 900 с.
  Две находки проверены по коду и подтвердились (ADR-0170). Правило `codex-adviser.md`
  снова исполнимо. История отказов ниже оставлена — рецидив возможен.
- **⚠️ 01.09.2026 — СОВЕТНИК НЕ РАБОТАЛ.** Три вызова подряд (постоянная сессия и два
...
**РЕШЕНО ИНАЧЕ (02.09):** модели взяты из набора **Kenney Furniture Pack**, который УЖЕ лежал на
машине — `~/scout-scenes/kits/kenney` (120 моделей, используется `schema_make.py`/`schema3d.py`).
Лицензия **CC0**: коммерческое использование без ограничений и БЕЗ обязательной ссылки
(`License.txt`), то есть строка с авторами на странице не нужна. Готовы к встройке:
`televisionModern` (72 грани), `doorway`/`doorwayFront` (188/156 — отдельная дверь с коробкой).
Окно `wallWindow`/`wallWindowSlide` и `wallDoorway` идут КУСКОМ СТЕНЫ 10×12.9 — поверх нашей
стены такую панель не поставить, нужен обрез до рамы или своя параметрическая рама.
Встроен телевизор: `kit_fixtures.py` печёт цвет материала в грани и выкладывает GLB в галерею
мешей (`/test/mesh-pilot10/kit-tv-modern/`), сцена достаёт его обычным `_scene3d_glb`;
соответствие роли и разворот — `draft_render.FIXTURE_MESH`.

**БЛОКЕР (историческое):** скачивание со Sketchfab требует входа в аккаунт — у агента его нет. Дверь, выбранная
...
## Рендер-сервис: фоновые задания, реестр кадров, миниатюры (02.09)
- **Фоновые задания.** `POST /api/render` с `async:1` возвращает `{job:<id>}` сразу; состояние —
  `GET /api/job?id=<id>`. Причина: мобильный браузер рвёт минутный запрос («Load failed»), сервер
  при этом досчитывает и отдаёт 200 — деньги потрачены, человек видит ошибку. Воркер ходит ЧЕРЕЗ
  СЕБЯ ЖЕ (`http://127.0.0.1:$DRAFT_PORT/render`), чтобы не дублировать маршрутизацию (прокси на
  DEV + локальный фолбэк). Требует МАРШРУТА `/api/job*` в `/opt/remlab/caddy/Caddyfile` — без
  него опрос даёт 404. `draft_service.py` живёт в двух местах: репозиторий (DEV) и контейнер
  `remlab-draft` (`docker cp` + `docker restart`), обновлять надо ОБА.
- **Кеш результата** по `src_id`: `SRC_DIR/<id>/result.json`; повтор с тем же исходником отдаёт
  готовое и не платит второй раз (`draft_render._improve_cache`).
- **Реестр кадров** — `/opt/remlab/test/share/shots.json` (в контейнере `/app/share/shots.json`),
  пишет `draft_service._registry_add()`: стиль, вариант, ссылки на ракурсы и миниатюры, состав
  товаров (`_group_items` схлопывает слоты одного товара и считает количество по упаковке),
  ссылка на исходник. Страница читает его при загрузке — до этого связь «кадр ↔ стиль» жила
  только в localStorage, и с другого устройства коллекция была пуста.
- **Миниатюры** пишет `draft_render._publish_frame` рядом с кадром по правилу имени
  (`кадр.jpg` → `кадр.t.jpg`, ширина 640). Карточки грузили полноразмерные кадры по 0,5–0,9 МБ;
  после правки коллекция весит ~140 КБ вместо 2,6 МБ.
