...
-- Лежала в `db/init/008-*` (схема БОЕВОЙ базы) и роняла автодеплой 01–05.09 — там нет и не должно
-- быть таблицы `products`. Граница слоёв — ADR-0179.
-- ЗАВИСИМОСТЬ: `asset_revisions` и `mesh_demand` создаёт `tools/scout/mesh_queue.py` (SCHEMA) в
-- рантайме, не миграцией. На живой дев-БД они есть; на ЧИСТОЙ этот файл упадёт, пока
-- control-plane DDL не переедет в миграции (долг, план `deploy-db-init-scope-fix` → follow-up).
--
