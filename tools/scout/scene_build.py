...
def derived(room, placements, items):
    ...
    if 'тв-тумба' in by and 'диван' in by:
        rel['тв-тумба'] = ('стоит у стены НАПРОТИВ дивана, фасадом к нему — это ТВ-зона, '
                           'диван и тумба смотрят друг на друга через комнату')
    if 'пуф' in by:
