...
ракурса, в котором снята карточка. Сильно — от 30° — значит камера видит его со стороны,
...
MIN_YAW = float(os.environ.get('MESH_MIN_YAW', 30))   # сильный разворот — от этого угла
...
def turn_deg(p, it, cam) -> float:
    ...
    Ракурс карточки — не всегда фронтальный: по замеру 5 420 товаров 53% сняты в три четверти
    (`enrich_bridge.photo_yaw`, 2026-08-05). Раньше это игнорировалось, и предмет, стоящий в сцене
    вполоборота — то есть ровно так, как он снят, — считался «сильно развёрнутым» и уезжал на
    платную 3D-модель. Теперь вычитаем собственный угол карточки; знак неизвестен, поэтому берём
    лучший из двух вариантов.
    """
    yaw = abs(mesh_yaw_pitch(p, it, cam)[0])
    try:
        from enrich_bridge import photo_yaw as _pyaw
        own = _pyaw(it.get('mid'), it.get('eid')) if isinstance(it, dict) else None
    except Exception:  # noqa: BLE001 — нет обогащения: работаем как раньше
        own = None
    if own:
        yaw = min(abs(yaw - own), abs(yaw + own))
