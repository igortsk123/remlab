...
def pair_prompt(n: int, cams: tuple[str, str], legends: list[list[dict]]) -> str:
    ...
    return (
        ...
        '- SIZE. On image 2 the KEY items (furniture and TV) are boxed with a red rectangle: the '
        'box is the exact extent of that item in the frame. The item must fit its box — same '
        'width and height, never bigger. Items without a box keep the size they have in the '
        'collage.\n'
        '- ROTATION. On the floor of image 1 every floor item has its true footprint outlined with '
        'a thin dark line: the base of the item must sit exactly on that outline, which fixes how '
        'the item is turned.\n'
