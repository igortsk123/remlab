...
def pair_prompt(n: int, cams: tuple[str, str], legends: list[list[dict]]) -> str:
    ...
    return (
        ...
        '- FOOTPRINTS. On the floor of image 1 the key furniture has thin dark rectangles: each is '
        'the TRUE BASE of that item — its real size on the floor and its rotation. Set the item so '
        'that its base fills exactly that rectangle: not wider, not longer, not turned differently. '
        'These rectangles are guides, never draw them in the output.\n'
