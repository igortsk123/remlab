...
def pair_prompt(n: int, cams: tuple[str, str], legends: list[list[dict]],
    ...
    return (
        ...
        '2 — the same sheet with red numbers and, above each number, the item\'s real size in '
        'centimetres as width×depth×height (annotation only, the same number is the same item in '
        'both frames; use those numbers to judge scale, never draw them). 3 — the floor plan with both camera positions and their fields of view.'
        ...
        'material from "appearance.material" — when that field is missing, read the material off '
        'the product photo in image 4; official colour from "appearance.colour_name"; '
        ...
        'or shifted off it; an item is replaced, recoloured or resized; an item is drawn in a '
        'material or colour that contradicts its "appearance" fields, or out of scale with its '
        'size_cm; an object appears that is neither in the item list nor in ALLOWED GENERATED '
        'ELEMENTS; an item shows up in the wrong frame; a window or door appears where the '
