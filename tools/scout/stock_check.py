...
15% — подтверждения (`suspect` ждёт второго голоса), ВСЕ товары боевых сетов, у которых истёк
суточный TTL, и хвост по давности на остаток. Снятые товары тоже проверяются, иначе ошибочное
снятие необратимо: `linkcheck` выбирал `where in_stock` и воскресить товар не мог.

ТОВАРЫ СЕТОВ — БЕЗ ПОТОЛКА (01.09, требование владельца «определять каждый день»). Была доля
20% (=700 при бюджете 3500) против 736 уникальных товаров в банках: примерно три десятка
позиций КАЖДЫЙ день выпадали из гарантированной квоты и уезжали в общий хвост — то есть
«ежедневно» для витрины не выполнялось. Доля бюджета — плохая мера там, где важен полный обход
маленького и самого дорогого множества: банк проверяем целиком, хвост забирает остаток.
