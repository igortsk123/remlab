...
import re as _re
CORNER=bool(items.get('диван')) and (
    ...
    if 'диван' in fd:
        ...
        if CORNER:
            ...
            if 'тв-тумба' in fd:
                ...
                if dist>TVD[1]: base=RD-min(dist-TVD[1],150)
            ...
            # ТВ-дистанция: далеко → диван отплывает от стены (float, свод р.2)
            if 'тв-тумба' in fd:
                d_tv=fd['тв-тумба'][1]
                dist=sofa_front-d_tv
                if dist>TVD[1]: 
                    sh=min(dist-TVD[1],120); sofa_front-=sh
