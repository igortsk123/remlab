...
SETS_FILE=(os.environ.get('SETS_SNAPSHOT') if (os.environ.get('SETS_SNAPSHOT') and '--v3' in sys.argv) else ('sets3.json' if '--v3' in sys.argv else ('sets2.json' if '--v2' in sys.argv else 'sets.json')))   # SETS_SNAPSHOT: замороженный банк экзамена (17.08)
...
# top-down PNG — ЕДИНЫЙ рендер из артефакта (render_plan.py; тот же код — для render-only без
