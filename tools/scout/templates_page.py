#!/usr/bin/env python3
"""УСТАРЕЛО 22.08.2026 — страница /test/templates удалена по решению владельца.

Заменена библиотекой канонов `canon_gallery.py` → /test/canons/: там схема рисуется в мини-комнате
с настоящим якорем боевыми плейсерами, проходит боевой валидатор и несёт паспорт (якорь/форма/
когда/почему/статус). Эта витрина показывала только габарит блока и не могла показать дефект
поиска; после слияния зеркал и переименований (ADR-0113) её имена схем разошлись с паспортом.
Файл оставлен как история; не публиковать.

Старое описание: витрина шаблонов зон на согласование владельцу (заявка 10.08): каждый шаблон
...
card('dining_6', 'Столовая: стол + 6 стульев',
     build_dining({'стол обеденный': _mk('стол обеденный', 160, 90),
                   **{k: v for k, v in CHAIRS.items()}}, 6),
     '4 по длинным сторонам + 2 торцевых. Идеал стола 160×90 (а не 180): по книжной '
     'норме 61 см кромки на едока шестеро помещаются, зато выбор в каталоге вырос '
     'в 7 раз (подгонка идеалов под фиды, решение владельца 11.08).',
     gross_m2=45, pos='center')
...
card('fireplace_solo', 'ЗОНА ИЗ ОДНОГО: камин соло',
     ...
     'Каскад закрыл 88 сцен, где каминная зона раньше не находила места.')

card('bulky_armchairs', 'Крупные кресла вторым рядом (кресла-кровати)',
     build_block('sofa_2armchairs',
                 {'диван': SOFA, 'кресло': _mk('кресло', 118, 119),
                  'кресло 2': _mk('кресло 2', 118, 119), 'столик': TBL, 'ковёр': RUG},
                 variant='bulky'),
     'Когда кресло по глубине почти как диван (от 100 см), флангом оно не встаёт '
     'физически: круг беседы против дистанции до столика. Схема ставит их вторым '
     'рядом напротив дивана, развёрнутыми к зоне.', gross_m2=32)

card('sofa_pouf_companion', 'Пуф-компаньон в посадочном шаблоне',
     build_block('sofa_armchair', {'диван': SOFA_S, 'столик': TBL_S,
                                   'пуф': _mk('пуф', 67, 50), 'ковёр': RUG_S}),
     'Пуф встроен в сам шаблон посадки — у свободного фланга на линии столика '
     '(веб-свод: «рядом или перед диваном», 35–45 см). Отдельная зона пуфа '
...
card('storage_single', 'ЗОНА ИЗ ОДНОГО: комод у стены',
     ...
     'Правило владельца: зона может состоять из одного предмета. Одиночный комод '
     '— полноценная зона хранения; декор на столешнице ставит рендер (по своду '
     'носитель несёт ~2 предмета).')

card('storage_pair', 'Хранение: комод + стеллаж (ряд из двух)',
     build_storage({'комод': _mk('комод', 120, 45), 'стеллаж': _mk('стеллаж', 80, 35)}),
     'Ряд из двух: открытые полки дают вертикаль, комод — вес у пола (веб-свод); '
     'фасады в одну линию, зазор 8 см.')

card('pouf_as_table', 'Крупный пуф ВМЕСТО столика (от 70 см)',
     build_block('sofa_2armchairs',
                 {'диван': SOFA, 'кресло': ARM, 'кресло 2': ARM2,
                  'пуф': _mk('пуф', 77, 102), 'ковёр': RUG}, variant='pouf_table'),
...
card('storage_wall', 'Стеллаж-стена: хранение в линию + растение (v2.4)',
     ...
     'ТВ-стене (межзонный штраф). Идеал секции 80 см (вдвое больше SKU, чем 90; '
     'ширину ряда задаёт число секций, не размер одной).')
