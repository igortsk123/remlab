...
<div class="head">Позицию и поворот блока в комнате выбирает солвер (у стены,
с пробой «есть ли напротив место под ТВ»); внутренность блока не перебирается и
уехать не может. Если блок не встаёт в геометрию комнаты — прежний поштучный
перебор (страховка «0 хуже»). Замечания пишите по названию шаблона — правки
вносятся в код шаблона и действуют на все будущие раскладки.</div>
{''.join(items_html)}
