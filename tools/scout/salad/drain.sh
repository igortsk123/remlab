...
for d in "${DIRS[@]}"; do
  ...
  # СРАВНИВАЕМ ОБЪЁМ, А НЕ ФАКТ НАЛИЧИЯ. Один и тот же комплект может быть перевыложен
  # ПОЛНЕЕ: у забракованных гейтом лежал только shape.glb, а после снятия гейтов та же
  # ревизия приезжает уже с model.glb. Проверка «есть локально → пропустить» такие обновления
  # не забирала вовсе (01.09: шесть позиций демо сгенерировались, но не доехали).
  if [ -f "$LOCAL/$d/complete.json" ]; then
    lsz=$(du -sb "$LOCAL/$d" 2>/dev/null | cut -f1)
    rsz=$(ssh -p "$PORT" "$SRV" "du -sb '$REMOTE/$d' 2>/dev/null | cut -f1")
    if [ -n "$lsz" ] && [ -n "$rsz" ] && [ "$lsz" -ge "$rsz" ]; then
      SKIP=$((SKIP + 1))
      if [ "$KEEP" != "--keep" ]; then ssh -p "$PORT" "$SRV" "rm -rf '$REMOTE/$d'"; fi
      continue
    fi
    echo "== $d: на сервере полнее ($rsz против $lsz) — забираю заново"
  fi
