...
# Правило владельца 01.09: НЕ БОЛЬШЕ 0.45 ступени, а меньше — сколько показал замер
# (долевого урезания нет: если товару нужно 0.2, столько и даём). Потолок стоит потому,
# что цель первого опыта — понять, реагирует ли покраска вообще, а не вылечить всё сразу:
# грубый сдвиг ломает вход, аккуратный — нет.
HARD = float(os.environ.get('EV_MAX', 0.45))


def _final(x: float) -> float:
    return round(float(min(max(x, -HARD), HARD)), 2)
