...
def cull_verdict(age_s: float, progress: float, since_move_s: float,
    ...
    # ДОКАЧАВШУЮ НОДУ НЕ СНИМАЕМ НИКОГДА (05.09). Правило судило по ТЕМПУ, не спросив, осталось
    # ли что качать: в логе `нода 674e0d46: 100%, темп 0.9%/мин → ещё 0 мин — ПЕРЕСАЖИВАЮ` и
    # `100%, темп 0.0%/мин → ещё 999 мин`. У готовой машины прогресс стоит по определению —
    # значит и «нет движения», и «темп ниже нормы» срабатывают на ней всегда. Цена ошибки: мы
    # выбрасываем машину, которая только что бесплатно скачала 26 ГБ, и ставим на её место
    # холодную — то есть платим за прогрев дважды и теряем 30–40 минут ёмкости.
    # Зависшую на 100 % машину (докачала, но не работает) снимают ДРУГИЕ правила — по здоровью
    # (`cull_dead_warmups`, `ssh_run.silent_fault`), а не по прогрессу закачки.
    if progress >= 0.999:
        return None
