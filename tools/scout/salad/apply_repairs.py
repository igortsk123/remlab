...
def rounds() -> None:
    ...
    import sys
    import time as _t
    # СКОЛЬКО РАУНДОВ — ЭТО ПРОПУСКНАЯ СПОСОБНОСТЬ, А НЕ ПАМЯТЬ (05.09).
    # Память ограничивает ACCEPT_CAP: каждый раунд — свежий дочерний процесс, и после него
    # система забирает память обратно (trimesh её не отдаёт внутри процесса, урок 391).
    # Раундов было 4 => 20 приёмок за цикл; цикл конвейера идёт раз в MESH_POST_EVERY_S=900 с,
    # то есть 80 приёмок в час против ~110 генерируемых мешей. Дефицит копился: на 05.09 в 10:20
    # на приёмнике лежало 330 комплектов, и чистка не могла удалить НИ ОДНОГО — «нет в базе 330»,
    # потому что без вердикта приёмки база не проставляет mesh_status. Приёмник рос до красной
    # черты, и пул вставал целиком. То есть узкое место всей цепочки — здесь, а не в уборке.
    # 12 раундов ≈ 60 приёмок за цикл (~2.4 мин на раунд по замеру) — с запасом над генерацией.
    n_rounds = int(os.environ.get('ACCEPT_ROUNDS', 12))
