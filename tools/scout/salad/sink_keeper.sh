...
# ПРИЁМКА — УЗКОЕ МЕСТО ВСЕЙ ЦЕПОЧКИ (замер 04.09). Удалять с транзита можно только то, что
# приёмка признала и база пометила; при 20 мешах за прогон против ~100 генерируемых в час
# очередь растёт, и приёмник переполняется снова. Разгоняем ПРАВИЛЬНОЙ ручкой: `ACCEPT_CAP`
# держит ПАМЯТЬ (сколько тяжёлых приёмок в одном процессе — его не трогаем, это защита от
# earlyoom), а `ACCEPT_ROUNDS` держит ПРОПУСКНУЮ СПОСОБНОСТЬ: каждый раунд — свежий дочерний
# процесс, память возвращается системе между раундами. Замер: раунд ≈ 2.4 мин на 5 мешей.
export ACCEPT_ROUNDS="${ACCEPT_ROUNDS:-8}"
export ACCEPT_TOTAL_BUDGET_S="${ACCEPT_TOTAL_BUDGET_S:-1500}"
