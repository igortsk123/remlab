...
# СВЕТ: множитель к текселю = LIGHT_A + LIGHT_B*|n·light|. Сумма ниже единицы означает, что товар
# на кадре систематически темнее, чем на фотографии магазина.
LIGHT_A = float(os.environ.get('SCENE3D_LIGHT_A', '0.62'))
LIGHT_B = float(os.environ.get('SCENE3D_LIGHT_B', '0.45'))
# СВЕТ УМНОЖАЕМ ПРАВИЛЬНО (нашёл Codex 02.09). Текстура хранится в sRGB — это НЕ линейные
# величины, и умножать их на коэффициент света нельзя: полутона проваливаются. Правильно —
# перевести в линейный свет, умножить, вернуть обратно. Точный путь стоил бы преобразования на
# каждый пиксель; вместо этого возводим САМ КОЭФФИЦИЕНТ в степень 1/2,4 — для умножения это
# почти то же самое и бесплатно (коэффициент один на грань).
# Замер: нынешний способ отличается от правильного в среднем на 10,8 из 255 (макс 48);
# приближение — на 1,1 (макс 7,7). Заодно перепад света становится честными 0,79 EV вместо 1,89.
LIGHT_LINEAR = os.environ.get('SCENE3D_LIGHT_LINEAR', '1') == '1'
_LIGHT_POW = (1.0 / 2.4) if LIGHT_LINEAR else 1.0
...
def raster_mesh(img, zbuf, parts, place, cam, W, Hpx, front_yaw: float,
    ...
    for mesh, world in zip(parts, worlds):
        ...
        shade = ((LIGHT_A + LIGHT_B * lam) ** _LIGHT_POW)[:, None]
        ...
        if SMOOTH:
            try:
                ...
                vsh = (LIGHT_A + LIGHT_B * np.clip(np.abs(vn @ light), 0, 1)) ** _LIGHT_POW
