...
ALLOW = [
    ...
    # и форма НАЗВАНИЯ товара для смешанных веток (MIXED): «Стол TC белый 80х75», «Кухонный стол Таргет»
    (r'кухонн\w* стол|стол обеденн|обеденн\w* стол|^стол\s', 'стол обеденный'),
    (r'\bстул', 'стул'),
...
# 03.09: после пересборки таксономии роли «стол обеденный» не осталось ни у одной ветки (единственная
# была у закрытого nonton), а столы лежат в СМЕШАННЫХ ветках: tvoydom «Столы и столики» (обеденные +
# журнальные), divan.ru «Столы и обеденные группы» (столы + стулья; целиком читалась как «стул»).
MIXED = {'112923:334', '99272:6983', '112923:30'}
...
def selftest() -> int:
    ...
    check('mixed: обеденный стол по названию', role_by_name('Стол TC белый 80х75 см') == 'стол обеденный', role_by_name('Стол TC белый 80х75 см'))
    check('mixed: кухонный стол', role_by_name('Кухонный стол divan.ru Таргет 90 Белый') == 'стол обеденный', role_by_name('Кухонный стол divan.ru Таргет 90 Белый'))
    check('mixed: журнальный столик', role_by_name('Столик журнальный Glasar 60') == 'столик', role_by_name('Столик журнальный Glasar 60'))
    check('mixed: стул', role_by_name('Стул divan.ru Франс Рогожка') == 'стул', role_by_name('Стул divan.ru Франс Рогожка'))
    check('путь «Столы и столики» не становится обеденным', classify('Мебель / Мебель для дома / Столы и столики')[0] == 'столик')
