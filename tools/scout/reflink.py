...
def direct(url):
    ...
    # ОБРЕЗАЕМ ТОЛЬКО ТАМ, ГДЕ КАРТОЧКА ДЕЙСТВИТЕЛЬНО МЕРТВА (26.08, находка владельца: «по этому
    # товару открывается divanboss.ru/stulya/ без указания на товар»). Проверено в тот же день:
    # у divanboss полный адрес карточки отдаёт 200 и НУЖНЫЙ товар, а обрезанный — соседний
    # («Стул НОРД» вместо «Стул АСТИ»); у mnogomebeli и карточка, и её родитель дают 404, живёт
    # только страница серии. Значит правило — пошаговое, не общее.
    if 'mnogomebeli' in host:
