...
def main() -> None:
    ...
    head = ("<div class='head'>Каждый канон нарисован ТЕМ ЖЕ кодом, что и рабочие планы: это не "
            ...
            "камин, угол, стена). Под планом — паспорт: когда применяется, почему так, статус."
            "<br><b>Как читать карточку (доктрина владельца, ADR-0117):</b> группа стоит НЕ по "
            "центру — это ситуационный канон: смещение и есть содержание («предмет + такое окно», "
            "«набор именно так ставится в угол») — в планах схема применяется только при этом "
            "якоре. Пунктирные предметы — референс отношений: как ДРУГИЕ объекты стоят "
            "относительно целевой группы (диван напротив носителя, экран на тумбе, дистанции). "
            "Группа ПО ЦЕНТРУ — ситуационной привязки нет: чистая композиция, позицию в комнате "
            "дают общие правила (фокус-стена, маршруты, ось).</div>")
