...
def scene_from_request(payload: dict) -> tuple:
    ...
    if stand is not None and not any(_base_role(p.role) == 'тв' for p in placements):
        ...
        # ГЛУБИНА — ПО СПОСОБУ УСТАНОВКИ (владелец 02.09: «телевизор стоит задом наперёд»).
        # На деле он стоял ЛИЦОМ — замер красным экраном это подтвердил, — но при глубине 8 см
        # у модели с подставкой ножка и шея сплющиваются в плоскость экрана, начинают спорить с
        # ним за глубину и проступают сквозь панель. Читается это ровно как вид сзади.
        # Панель на стене — 8 см; телевизор на тумбе — с подставкой, ей нужно место.
        depth = 8.0 if _hung_stand(stand) else 22.0
...
def _scene3d_parts(glb: str):
    ...
    СЛУЖЕБНЫЕ ФОРМЫ (окно `win:…`, телевизор `tv:…`) строятся кодом, а не читаются с диска, и
    кладутся в тот же кэш ПО СВОЕМУ КЛЮЧУ — строкой. Когда 02.09 у кэша появился второй ключ
    (лёгкие меши против полных), эти записи перестали находиться: загрузчик шёл искать файл
    `win:150x125`, не находил и молча пропускал предмет — с кадра исчезли окно и телевизор.
    Поэтому сперва смотрим по «сырому» ключу, и только потом по паре с режимом.
    """
    import mesh_render as MR
    raw = _S3_PARTS.get(glb)
    if raw is not None:
        return raw
