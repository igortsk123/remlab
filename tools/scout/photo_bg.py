#!/usr/bin/env python3
"""Фон фото: белая карточка или нет. Небелый — метка «возможно коллаж, на проверку».

ЗАЧЕМ (владелец 01.09): «только то, что не на белом фоне, помечай как возможно коллаж на
проверку, и всё». Метка НЕ утверждает, что это коллаж, — она отправляет товар на глаза
человеку. Модель при этом делается всегда: гейт вырезки больше не блокирует.
...
ПОЧЕМУ МЕТКА «ВОЗМОЖНО», А НЕ «КОЛЛАЖ». Отделить рекламный баннер от честной съёмки в
интерьере надёжно не выходит: на баннере стола TC Vox надписи светлые на светлой стене и по
плотности границ (6%) не отличаются от складок шторы (4%). Ранняя версия считала текстом всё
тёмное по краям и записывала в коллажи интерьерную съёмку подушки (54% «текста») — поймано
глазами. Поэтому решает человек, а метка лишь показывает ему, что смотреть.
...
def main() -> None:
    ...
    for sku, name, url, mesh_status in rows:
        ...
        mark = 'НА ПРОВЕРКУ' if r['collage'] else 'белый фон'
    ...
    print(f'\nпросмотрено {seen}, на проверку {collage}'
