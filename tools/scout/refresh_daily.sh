...
# 4b. ТОЧНЫЙ стиль-скоринг новинок (владелец 29.08): правила паспортов → CLIP → gpt-5-mini,
# балл 0–10 по 6 стилям + universal. Кэш по mid-eid — скорится только дельта, обычно единицы.
# Без него новинки живут на грубом фолбэке обогащения («высокая/низкая»), и стиль-фит сетов
# для них считается вслепую. Дневной предел — защита от случайной массовой дельты.
paid_step style_score "$PY" style_score.py --limit "${STYLE_DAILY_LIMIT:-300}"
