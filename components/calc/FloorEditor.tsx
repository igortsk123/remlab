...
// Крестик удаления зоны: без подчёркивания, приглушённый, у правого края.
const xBtn = {
...
// Редактор пола (ламинат/плитка-пол): длина×ширина + дополнительные и исключаемые зоны.
// Зоны — длина и ширина В СТРОКУ (не в столбик), крестик справа.
export function FloorEditor({ floor, onChange }: { floor: Floor; onChange: (f: Floor) => void }) {
    ...
    <div className="stack" style={{ gap: 6 }}>
      {floor[key].map((z) => (
        <div key={z.id} className="row" style={{ gap: 8, alignItems: "center", flexWrap: "nowrap" }}>
          ...
          <button type="button" style={xBtn} aria-label="удалить зону" onClick={() => patch({ [key]: floor[key].filter((x) => x.id !== z.id) } as Partial<Floor>)}>×</button>
        </div>
      ))}
  ...
  return (
    <div className="stack" style={{ gap: 10 }}>
      ...
      <div className="stack" style={{ gap: 4 }}>
        <span className="eyebrow">Исключить зону</span>
        {zones("excludedZones")}
      </div>
