...
GROUPS = (frozenset({"диван", "столик", "кресло", "пуф"}),
          # Q6b свода №13: банкетка — член обеденной группы (уголок «банкетка+стол+стулья»):
          # зоны подхода членов одной группы друг друга не блокируют, иначе уголок невозможен
          # по построению (стол обязан стоять вплотную — ACCESS_BLOCKED на всех позициях)
          frozenset({"стол обеденный", "стул", "банкетка"}))
ZONE_GROUP = GROUPS[0]
