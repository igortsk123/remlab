...
def not_worse(before: dict, after: dict) -> bool:
    """ПОРОГОВАЯ политика (C-1 свода №11, уточнение семантики по аудиту Кодекса §10):
    имя историческое; фактический контракт — «В ПРЕДЕЛАХ ПОРОГОВ», не «не хуже было»:
    маршрут может сузиться до max-порога (120→75 легально), щели вырасти до +0.35,
    фокус уйти до 40. Это ОСОЗНАННОЕ решение ADR-0091 (строгое "не хуже" душило бы
    доборы зон). Менять поведение — только отдельным решением владельца.
    """
