...
class Camera:
    ...
    def basis(self):
        ...
        # right = fwd × up (01.09). Прежнее up × fwd давало кадр, ЗЕРКАЛЬНЫЙ плану, — владелец
        # увидел это на демо. Правка 2026-08-04 шла от словесного правила «лицом на север —
        # восток справа», но север/восток тут ничем не заданы: ориентацию задаёт САМ ПЛАН, а он
        # рисуется как вид сверху с осью y ВНИЗ экрана (`index.html`: X=x*S, Y=y*S).
        # Вывод из плана: наблюдатель смотрит по (dx,dz) — его право на плане это (-dz, dx),
        # то есть fwd × up, а не наоборот. Проверка численная (`--selftest`): наблюдатель у
        # нижнего края смотрит вверх по плану, предмет у правой стены обязан быть справа кадра.
        right = np.cross(fwd, up)
        if np.linalg.norm(right) < 1e-6:
            right = np.array([1.0, 0.0, 0.0])
        right /= np.linalg.norm(right)
        # true_up = right × fwd: со сменой знака right вертикаль иначе перевернулась бы вместе
        # с горизонталью, и кадр вместо зеркала стал бы поворотом на 180°
...
# ТОРШЕР — ТОНКАЯ СТОЙКА С ПЛАФОНОМ (27.08): голая коробка 38×22 читалась моделью как тумбочка, и
# на её место рисовался стеллаж, а сам торшер уезжал к дивану (кадр 220fbeb52e, разбор владельца).
_FLOOR_LAMP = [(0.25, 0.75, 0.25, 0.75, 0.0, 0.04),    # блин основания
               (0.45, 0.55, 0.45, 0.55, 0.04, 0.82),   # стойка
               (0.05, 0.95, 0.05, 0.95, 0.82, 1.0)]    # плафон
# СТУЛ — сиденье, спинка и четыре ножки: коробка со спинкой читалась как тумба или кресло.
_CHAIR = [(0.06, 0.94, 0.06, 0.94, 0.45, 0.52),        # сиденье
          (0.06, 0.94, 0.86, 1.0, 0.52, 1.0),          # спинка
          (0.08, 0.18, 0.08, 0.18, 0.0, 0.45),         # ножки
          (0.82, 0.92, 0.08, 0.18, 0.0, 0.45),
          (0.08, 0.18, 0.82, 0.92, 0.0, 0.45),
          (0.82, 0.92, 0.82, 0.92, 0.0, 0.45)]
...
PROXY: dict[str, list[tuple[float, ...]]] = {
    ...
    # роли нашего каталога, у которых прокси не было и коробка врала о смысле (27.08)
    "стол обеденный": _TABLE, "приставной": _TABLE, "стол-консоль": _TABLE,
    "стул": _CHAIR, "банкетка": _SEAT, "пуф": _SEAT,
    "торшер": _FLOOR_LAMP, "лампа": _FLOOR_LAMP, "светильник": _FLOOR_LAMP,
    "витрина": _SHELF, "камин": _CASE,
}
...
def compile_scene(room: Room, placements: list[Placement], cam: Camera) -> dict:
    ...
    # СТЕНА МЕЖДУ КАМЕРОЙ И КОМНАТОЙ НЕ РИСУЕТСЯ (27.08): так снимают интерьеры — объектив стоит
    # «в стене», иначе камера в углу упирается в мебель и половина предметов уходит за рамку.
    ex, _, ez = (float(v) for v in cam.eye)
    skip = {"north": ez > D, "south": ez < 0, "west": ex < 0, "east": ex > W}
    for wall, quad in (
        ("north", [[0, 0, D], [W, 0, D], [W, H, D], [0, H, D]]),
        ("west", [[0, 0, 0], [0, 0, D], [0, H, D], [0, H, 0]]),
        ("east", [[W, 0, 0], [W, 0, D], [W, H, D], [W, H, 0]]),
        ("south", [[0, 0, 0], [W, 0, 0], [W, H, 0], [0, H, 0]]),
    ):
        if skip[wall]:
            continue
    ...
    for op in room.openings:
        if skip.get(op.wall):          # проём на срезанной стене висел бы в воздухе
            continue
...
def clay_render(out: dict) -> np.ndarray:
    ...
    # ФОН — НЕ ЧЁРНЫЙ (27.08): при срезанной ближней стене часть кадра не попадает ни на что, и
    # чёрные поля модель принимала за тёмную стену. Красим их светлым нейтралом.
    img[sem == 0] = (238, 236, 232)
    d = depth.copy()
    fin = np.isfinite(d)
    if fin.any():
