...
def test_large_room_distance_bounded():
    ...
    share = far / tot
    assert share <= 0.42, (      # 17.08: 41.1% после ремапа ролей каталога (кресла/пуфы divan.ru
        # вернулись в пул → другие банки премиум-сетов). ЗАФИКСИРОВАННЫЙ ДОЛГ: планка обязана
        # вернуться к 41 и ниже в пакете дистанций ТВ (см. MASTER-zones-v7, открытые вопросы).
