...
def manual_layout(room: Room) -> list[Placement]:
    ...
    if gap > tv_hi:
        off = gap - tv_hi
        # Q8 (17.08): полоса за спинкой — прижат (<15) | воздух 15–30 | проход ≥91;
        # промежуточное 31–90 пусто = щель (SOFA_SLIVER). Раньше эталон «отплывал» на 76 —
        # это ровно бесхозная полоса, которую владелец и забраковал в галерее
        if 31 <= off < 91:
            off = 30.0 if off < 60 else 91.0
        sofa_y -= off
        sofa_front = sofa_y - sofa.d_cm / 2
